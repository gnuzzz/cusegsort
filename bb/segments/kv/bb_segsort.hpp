...
#ifndef _HPP_BB_SEGMENTS_KV_SEGSORT
#define _HPP_BB_SEGMENTS_KV_SEGSORT
  ...
  namespace segments {
    namespace kv {
      ...
      int bb_segsort(K *keys_d, T *vals_d, int n, int *d_segs, int length) {
        SortContext<K, T> context(n, length);
        return bb_segsort(keys_d, vals_d, n, d_segs, length, &context);
      }
