...
#ifndef _HPP_BB_MATRIX_KV_COMPUT_L
#define _HPP_BB_MATRIX_KV_COMPUT_L

#include <thrust/device_ptr.h>
#include <thrust/scan.h>
#include <limits>

#include "../../kv/bb_exch.hpp"
...
namespace bb {
  namespace matrix {
    namespace kv {
      ...
      __global__
      void kern_block_sort(K *key, T *val, K *keyB, T *valB,
                           int *bin, int *blk_innerid, int *blk_seg_start, const int rows, const int cols) {
        /*** codegen ***/
        ...
        CMP_SWP_KV(K, rg_k0, rg_k1, int, rg_v0, rg_v1);
        CMP_SWP_KV(K, rg_k2, rg_k3, int, rg_v2, rg_v3);
        // exch_intxn: switch to exch_local()
        CMP_SWP_KV(K, rg_k0, rg_k3, int, rg_v0, rg_v3);
        CMP_SWP_KV(K, rg_k1, rg_k2, int, rg_v1, rg_v2);
        ...
        exch_intxn(rg_k0, rg_k1, rg_k2, rg_k3,
                   ...
                   0x1, bit1);
        // exch_paral: switch to exch_local()
        CMP_SWP_KV(K, rg_k0, rg_k2, int, rg_v0, rg_v2);
        CMP_SWP_KV(K, rg_k1, rg_k3, int, rg_v1, rg_v3);
        // exch_paral: switch to exch_local()
        CMP_SWP_KV(K, rg_k0, rg_k1, int, rg_v0, rg_v1);
        CMP_SWP_KV(K, rg_k2, rg_k3, int, rg_v2, rg_v3);
        // exch_intxn: generate exch_intxn()
      ...
      template<class K, class T>
      __global__
      void kern_block_merge(K *keys, T *vals, K *keysB, T *valsB, int *bin,
                            int *blk_innerid, int *blk_seg_start, const int rows, const int cols, int stride) {
        __shared__
        K smem[128 * 16];
        const int tid = threadIdx.x;
        const int bid = blockIdx.x;
        ...
        if (stride < seg_size) {
          ...
          if (lane_id & 0x1) SWP_KV(K, rg_k0, rg_k1, int, rg_v0, rg_v1);
          if (lane_id & 0x1) SWP_KV(K, rg_k2, rg_k3, int, rg_v2, rg_v3);
          if (lane_id & 0x1) SWP_KV(K, rg_k4, rg_k5, int, rg_v4, rg_v5);
          if (lane_id & 0x1) SWP_KV(K, rg_k6, rg_k7, int, rg_v6, rg_v7);
          if (lane_id & 0x1) SWP_KV(K, rg_k8, rg_k9, int, rg_v8, rg_v9);
          if (lane_id & 0x1) SWP_KV(K, rg_k10, rg_k11, int, rg_v10, rg_v11);
          if (lane_id & 0x1) SWP_KV(K, rg_k12, rg_k13, int, rg_v12, rg_v13);
          if (lane_id & 0x1) SWP_KV(K, rg_k14, rg_k15, int, rg_v14, rg_v15);
          rg_k1 = __shfl_xor(rg_k1, 0x1);
          rg_k3 = __shfl_xor(rg_k3, 0x1);
          rg_k5 = __shfl_xor(rg_k5, 0x1);
          rg_k7 = __shfl_xor(rg_k7, 0x1);
          ...
          if (lane_id & 0x2) SWP_KV(K, rg_k0, rg_k2, int, rg_v0, rg_v2);
          if (lane_id & 0x2) SWP_KV(K, rg_k1, rg_k3, int, rg_v1, rg_v3);
          if (lane_id & 0x2) SWP_KV(K, rg_k4, rg_k6, int, rg_v4, rg_v6);
          if (lane_id & 0x2) SWP_KV(K, rg_k5, rg_k7, int, rg_v5, rg_v7);
          if (lane_id & 0x2) SWP_KV(K, rg_k8, rg_k10, int, rg_v8, rg_v10);
          if (lane_id & 0x2) SWP_KV(K, rg_k9, rg_k11, int, rg_v9, rg_v11);
          if (lane_id & 0x2) SWP_KV(K, rg_k12, rg_k14, int, rg_v12, rg_v14);
          if (lane_id & 0x2) SWP_KV(K, rg_k13, rg_k15, int, rg_v13, rg_v15);
          rg_k2 = __shfl_xor(rg_k2, 0x2);
          rg_k3 = __shfl_xor(rg_k3, 0x2);
          rg_k6 = __shfl_xor(rg_k6, 0x2);
          rg_k7 = __shfl_xor(rg_k7, 0x2);
          ...
          if (lane_id & 0x4) SWP_KV(K, rg_k0, rg_k4, int, rg_v0, rg_v4);
          if (lane_id & 0x4) SWP_KV(K, rg_k1, rg_k5, int, rg_v1, rg_v5);
          if (lane_id & 0x4) SWP_KV(K, rg_k2, rg_k6, int, rg_v2, rg_v6);
          if (lane_id & 0x4) SWP_KV(K, rg_k3, rg_k7, int, rg_v3, rg_v7);
          if (lane_id & 0x4) SWP_KV(K, rg_k8, rg_k12, int, rg_v8, rg_v12);
          if (lane_id & 0x4) SWP_KV(K, rg_k9, rg_k13, int, rg_v9, rg_v13);
          if (lane_id & 0x4) SWP_KV(K, rg_k10, rg_k14, int, rg_v10, rg_v14);
          if (lane_id & 0x4) SWP_KV(K, rg_k11, rg_k15, int, rg_v11, rg_v15);
          rg_k4 = __shfl_xor(rg_k4, 0x4);
          rg_k5 = __shfl_xor(rg_k5, 0x4);
          rg_k6 = __shfl_xor(rg_k6, 0x4);
          rg_k7 = __shfl_xor(rg_k7, 0x4);
          ...
          if (lane_id & 0x8) SWP_KV(K, rg_k0, rg_k8, int, rg_v0, rg_v8);
          if (lane_id & 0x8) SWP_KV(K, rg_k1, rg_k9, int, rg_v1, rg_v9);
          if (lane_id & 0x8) SWP_KV(K, rg_k2, rg_k10, int, rg_v2, rg_v10);
          if (lane_id & 0x8) SWP_KV(K, rg_k3, rg_k11, int, rg_v3, rg_v11);
          if (lane_id & 0x8) SWP_KV(K, rg_k4, rg_k12, int, rg_v4, rg_v12);
          if (lane_id & 0x8) SWP_KV(K, rg_k5, rg_k13, int, rg_v5, rg_v13);
          if (lane_id & 0x8) SWP_KV(K, rg_k6, rg_k14, int, rg_v6, rg_v14);
          if (lane_id & 0x8) SWP_KV(K, rg_k7, rg_k15, int, rg_v7, rg_v15);
          rg_k8 = __shfl_xor(rg_k8, 0x8);
          rg_k9 = __shfl_xor(rg_k9, 0x8);
          rg_k10 = __shfl_xor(rg_k10, 0x8);
          rg_k11 = __shfl_xor(rg_k11, 0x8);
          ...
          if (lane_id & 0x10) SWP_KV(K, rg_k0, rg_k1, int, rg_v0, rg_v1);
          if (lane_id & 0x10) SWP_KV(K, rg_k2, rg_k3, int, rg_v2, rg_v3);
          if (lane_id & 0x10) SWP_KV(K, rg_k4, rg_k5, int, rg_v4, rg_v5);
          if (lane_id & 0x10) SWP_KV(K, rg_k6, rg_k7, int, rg_v6, rg_v7);
          if (lane_id & 0x10) SWP_KV(K, rg_k8, rg_k9, int, rg_v8, rg_v9);
          if (lane_id & 0x10) SWP_KV(K, rg_k10, rg_k11, int, rg_v10, rg_v11);
          if (lane_id & 0x10) SWP_KV(K, rg_k12, rg_k13, int, rg_v12, rg_v13);
          if (lane_id & 0x10) SWP_KV(K, rg_k14, rg_k15, int, rg_v14, rg_v15);
          rg_k1 = __shfl_xor(rg_k1, 0x10);
          rg_k3 = __shfl_xor(rg_k3, 0x10);
          rg_k5 = __shfl_xor(rg_k5, 0x10);
          rg_k7 = __shfl_xor(rg_k7, 0x10);
          ...
          if ((innerbid << 11) + (warp_id << 9) + 0 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 0 + lane_id] = rg_k0;
          if ((innerbid << 11) + (warp_id << 9) + 32 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 32 + lane_id] = rg_k2;
          if ((innerbid << 11) + (warp_id << 9) + 64 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 64 + lane_id] = rg_k4;
          if ((innerbid << 11) + (warp_id << 9) + 96 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 96 + lane_id] = rg_k6;
          if ((innerbid << 11) + (warp_id << 9) + 128 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 128 + lane_id] = rg_k8;
          if ((innerbid << 11) + (warp_id << 9) + 160 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 160 + lane_id] = rg_k10;
          if ((innerbid << 11) + (warp_id << 9) + 192 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 192 + lane_id] = rg_k12;
          if ((innerbid << 11) + (warp_id << 9) + 224 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 224 + lane_id] = rg_k14;
          if ((innerbid << 11) + (warp_id << 9) + 256 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 256 + lane_id] = rg_k1;
          if ((innerbid << 11) + (warp_id << 9) + 288 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 288 + lane_id] = rg_k3;
          if ((innerbid << 11) + (warp_id << 9) + 320 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 320 + lane_id] = rg_k5;
          if ((innerbid << 11) + (warp_id << 9) + 352 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 352 + lane_id] = rg_k7;
          if ((innerbid << 11) + (warp_id << 9) + 384 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 384 + lane_id] = rg_k9;
          if ((innerbid << 11) + (warp_id << 9) + 416 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 416 + lane_id] = rg_k11;
          if ((innerbid << 11) + (warp_id << 9) + 448 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 448 + lane_id] = rg_k13;
          if ((innerbid << 11) + (warp_id << 9) + 480 + lane_id < seg_size)
            keysB[k + (innerbid << 11) + (warp_id << 9) + 480 + lane_id] = rg_k15;

          if ((innerbid << 11) + (warp_id << 9) + 0 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 0 + lane_id] = vals[k + rg_v0];
          if ((innerbid << 11) + (warp_id << 9) + 32 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 32 + lane_id] = vals[k + rg_v2];
          if ((innerbid << 11) + (warp_id << 9) + 64 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 64 + lane_id] = vals[k + rg_v4];
          if ((innerbid << 11) + (warp_id << 9) + 96 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 96 + lane_id] = vals[k + rg_v6];
          if ((innerbid << 11) + (warp_id << 9) + 128 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 128 + lane_id] = vals[k + rg_v8];
          if ((innerbid << 11) + (warp_id << 9) + 160 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 160 + lane_id] = vals[k + rg_v10];
          if ((innerbid << 11) + (warp_id << 9) + 192 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 192 + lane_id] = vals[k + rg_v12];
          if ((innerbid << 11) + (warp_id << 9) + 224 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 224 + lane_id] = vals[k + rg_v14];
          if ((innerbid << 11) + (warp_id << 9) + 256 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 256 + lane_id] = vals[k + rg_v1];
          if ((innerbid << 11) + (warp_id << 9) + 288 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 288 + lane_id] = vals[k + rg_v3];
          if ((innerbid << 11) + (warp_id << 9) + 320 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 320 + lane_id] = vals[k + rg_v5];
          if ((innerbid << 11) + (warp_id << 9) + 352 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 352 + lane_id] = vals[k + rg_v7];
          if ((innerbid << 11) + (warp_id << 9) + 384 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 384 + lane_id] = vals[k + rg_v9];
          if ((innerbid << 11) + (warp_id << 9) + 416 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 416 + lane_id] = vals[k + rg_v11];
          if ((innerbid << 11) + (warp_id << 9) + 448 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 448 + lane_id] = vals[k + rg_v13];
          if ((innerbid << 11) + (warp_id << 9) + 480 + lane_id < seg_size)
            valsB[k + (innerbid << 11) + (warp_id << 9) + 480 + lane_id] = vals[k + rg_v15];
        }
      }

      template<class K, class T>
      __global__
      void kern_copy(K *srck, T *srcv, K *dstk, T *dstv, int *bin,
                     int *blk_innerid, int *blk_seg_start, const int rows, const int cols, int res) {
        const int tid = threadIdx.x;
        const int bid = blockIdx.x;
        int innerbid = blk_innerid[bid];
        int bin_it = blk_seg_start[bid];
        ...
        if ((steps & 1)) {
          ...
          if ((innerbid << 11) + tid + 128 < seg_size)
            dstk[k + (innerbid << 11) + tid + 128] = srck[k + (innerbid << 11) + tid + 128];
          if ((innerbid << 11) + tid + 256 < seg_size)
            dstk[k + (innerbid << 11) + tid + 256] = srck[k + (innerbid << 11) + tid + 256];
          if ((innerbid << 11) + tid + 384 < seg_size)
            dstk[k + (innerbid << 11) + tid + 384] = srck[k + (innerbid << 11) + tid + 384];
          if ((innerbid << 11) + tid + 512 < seg_size)
            dstk[k + (innerbid << 11) + tid + 512] = srck[k + (innerbid << 11) + tid + 512];
          if ((innerbid << 11) + tid + 640 < seg_size)
            dstk[k + (innerbid << 11) + tid + 640] = srck[k + (innerbid << 11) + tid + 640];
          if ((innerbid << 11) + tid + 768 < seg_size)
            dstk[k + (innerbid << 11) + tid + 768] = srck[k + (innerbid << 11) + tid + 768];
          if ((innerbid << 11) + tid + 896 < seg_size)
            dstk[k + (innerbid << 11) + tid + 896] = srck[k + (innerbid << 11) + tid + 896];
          if ((innerbid << 11) + tid + 1024 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1024] = srck[k + (innerbid << 11) + tid + 1024];
          if ((innerbid << 11) + tid + 1152 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1152] = srck[k + (innerbid << 11) + tid + 1152];
          if ((innerbid << 11) + tid + 1280 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1280] = srck[k + (innerbid << 11) + tid + 1280];
          if ((innerbid << 11) + tid + 1408 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1408] = srck[k + (innerbid << 11) + tid + 1408];
          if ((innerbid << 11) + tid + 1536 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1536] = srck[k + (innerbid << 11) + tid + 1536];
          if ((innerbid << 11) + tid + 1664 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1664] = srck[k + (innerbid << 11) + tid + 1664];
          if ((innerbid << 11) + tid + 1792 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1792] = srck[k + (innerbid << 11) + tid + 1792];
          if ((innerbid << 11) + tid + 1920 < seg_size)
            dstk[k + (innerbid << 11) + tid + 1920] = srck[k + (innerbid << 11) + tid + 1920];

          if ((innerbid << 11) + tid < seg_size) dstv[k + (innerbid << 11) + tid] = srcv[k + (innerbid << 11) + tid];
          if ((innerbid << 11) + tid + 128 < seg_size)
            dstv[k + (innerbid << 11) + tid + 128] = srcv[k + (innerbid << 11) + tid + 128];
          if ((innerbid << 11) + tid + 256 < seg_size)
            dstv[k + (innerbid << 11) + tid + 256] = srcv[k + (innerbid << 11) + tid + 256];
          if ((innerbid << 11) + tid + 384 < seg_size)
            dstv[k + (innerbid << 11) + tid + 384] = srcv[k + (innerbid << 11) + tid + 384];
          if ((innerbid << 11) + tid + 512 < seg_size)
            dstv[k + (innerbid << 11) + tid + 512] = srcv[k + (innerbid << 11) + tid + 512];
          if ((innerbid << 11) + tid + 640 < seg_size)
            dstv[k + (innerbid << 11) + tid + 640] = srcv[k + (innerbid << 11) + tid + 640];
          if ((innerbid << 11) + tid + 768 < seg_size)
            dstv[k + (innerbid << 11) + tid + 768] = srcv[k + (innerbid << 11) + tid + 768];
          if ((innerbid << 11) + tid + 896 < seg_size)
            dstv[k + (innerbid << 11) + tid + 896] = srcv[k + (innerbid << 11) + tid + 896];
          if ((innerbid << 11) + tid + 1024 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1024] = srcv[k + (innerbid << 11) + tid + 1024];
          if ((innerbid << 11) + tid + 1152 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1152] = srcv[k + (innerbid << 11) + tid + 1152];
          if ((innerbid << 11) + tid + 1280 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1280] = srcv[k + (innerbid << 11) + tid + 1280];
          if ((innerbid << 11) + tid + 1408 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1408] = srcv[k + (innerbid << 11) + tid + 1408];
          if ((innerbid << 11) + tid + 1536 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1536] = srcv[k + (innerbid << 11) + tid + 1536];
          if ((innerbid << 11) + tid + 1664 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1664] = srcv[k + (innerbid << 11) + tid + 1664];
          if ((innerbid << 11) + tid + 1792 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1792] = srcv[k + (innerbid << 11) + tid + 1792];
          if ((innerbid << 11) + tid + 1920 < seg_size)
            dstv[k + (innerbid << 11) + tid + 1920] = srcv[k + (innerbid << 11) + tid + 1920];
        }
      }

      template<class K, class T>
      int gen_grid_kern_r2049(K *keys_d, T *vals_d, K *keysB_d, T *valsB_d,
                              const int rows, const int cols, int *bin_d, int bin_size) {
        cudaError_t err;
        int *blk_stat_d; // histogram of how many blocks for each seg
        err = cudaMalloc((void **) &blk_stat_d, bin_size * sizeof(int));
        CUDA_CHECK(err, "alloc blk_stat_d");

        int blk_num; // total number of blocks
        int *max_segsize_d;
        err = cudaMalloc((void **) &max_segsize_d, sizeof(int));
        CUDA_CHECK(err, "alloc max_segsize_d");

        err = cudaMemset(max_segsize_d, 0, sizeof(int));
        CUDA_CHECK(err, "memset max_segsize_d");
        ...
        kern_get_num_blk_init << < grids, blocks >> > (max_segsize_d, bin_d, blk_stat_d,
            rows, cols, bin_size, 2048); // 512thread*4key /*** codegen ***/

        int max_segsize;
        err = cudaMemcpy(&max_segsize, max_segsize_d, sizeof(int), cudaMemcpyDeviceToHost);
        CUDA_CHECK(err, "copy from max_segsize_d");
        ...
        CUDA_CHECK(err, "copy from blk_stat_d+bin_size-1");
        ...
        blocks.x = 512;
        grids.x = blk_num;
        kern_block_sort << < grids, blocks >> > (keys_d, vals_d, keysB_d, valsB_d, bin_d,
            blk_innerid, blk_seg_start, rows, cols);

        blocks.x = 256;
        grids.x = (bin_size + blocks.x - 1) / blocks.x;
        kern_get_num_blk << < grids, blocks >> > (bin_d, blk_stat_d,
            rows, cols, bin_size, 2048); // 128t*16k /*** codegen ***/
        ...
        CUDA_CHECK(err, "free blk_innerid");

        err = cudaMalloc((void **) &blk_innerid, blk_num * sizeof(int));
        CUDA_CHECK(err, "alloc blk_innerid");

        err = cudaFree(blk_seg_start);
        CUDA_CHECK(err, "free blk_seg_start");
        ...
        while (stride < max_segsize) {
          kern_block_merge << < grids, blocks >> > (keys_d, vals_d, keysB_d, valsB_d, bin_d,
              blk_innerid, blk_seg_start, rows, cols, stride);
          stride <<= 1;
          std::swap(keys_d, keysB_d);
          std::swap(vals_d, valsB_d);
          cnt++;
        ...
        kern_copy << < grids, blocks >> > (srck, srcv, dstk, dstv, bin_d,
            blk_innerid, blk_seg_start, rows, cols, cnt);

        err = cudaFree(blk_stat_d);
        CUDA_CHECK(err, "free blk_stat_d");
        err = cudaFree(blk_innerid);
        CUDA_CHECK(err, "free blk_innerid");
        err = cudaFree(blk_seg_start);
        CUDA_CHECK(err, "free blk_seg_start");
        err = cudaFree(max_segsize_d);
        CUDA_CHECK(err, "free max_segsize_d");
