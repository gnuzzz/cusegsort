...
#ifndef _HPP_BB_MATRIX_KV_SEGSORT
#define _HPP_BB_MATRIX_KV_SEGSORT

#include "../../kv/bb_context.hpp"
#include "../bb_bin.hpp"
...
namespace bb {
  namespace matrix {
    namespace kv {
      ...
      int bb_segsort(K *keys_d, T *vals_d, const int rows, const int cols, const SortContext<K, T> *context) {
        ...
        T *valsB_d = context->valsB_d;

        bb_bin(d_bin_segs_id, d_bin_counter, rows, cols, h_bin_counter);
        ...
        if (subwarp_num > 0)
          gen_copy << < grids, blocks, 0, streams[0] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[0], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk256_wp2_tc1_r2_r2_orig << < grids, blocks, 0, streams[1] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[1], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_wp2_tc2_r3_r4_orig << < grids, blocks, 0, streams[2] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[2], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_wp2_tc4_r5_r8_orig << < grids, blocks, 0, streams[3] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[3], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_wp4_tc4_r9_r16_strd << < grids, blocks, 0, streams[4] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[4], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_wp8_tc4_r17_r32_strd << < grids, blocks, 0, streams[5] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[5], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_wp16_tc4_r33_r64_strd << < grids, blocks, 0, streams[6] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[6], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk256_wp8_tc16_r65_r128_strd << < grids, blocks, 0, streams[7] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[7], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk256_wp32_tc8_r129_r256_strd << < grids, blocks, 0, streams[8] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[8], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk128_tc4_r257_r512_orig << < grids, blocks, 0, streams[9] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[9], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk256_tc4_r513_r1024_orig << < grids, blocks, 0, streams[10] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[10], subwarp_num);
        ...
        if (subwarp_num > 0)
          gen_bk512_tc4_r1025_r2048_orig << < grids, blocks, 0, streams[11] >> > (keys_d, vals_d, keysB_d, valsB_d,
              rows, cols, d_bin_segs_id + h_bin_counter[11], subwarp_num);

        // sort long segments
        subwarp_num = rows - h_bin_counter[12];
        if (subwarp_num > 0)
          gen_grid_kern_r2049(keys_d, vals_d, keysB_d, valsB_d,
                              rows, cols, d_bin_segs_id + h_bin_counter[12], subwarp_num);
      ...
      template<class K, class T>
      int bb_segsort(K *keys_d, T *vals_d, const int rows, const int cols) {
        SortContext<K, T> context(rows * cols, rows);
        return bb_segsort(keys_d, vals_d, rows, cols, &context);
      }
