...
#ifndef _HPP_BB_MATRIX_BIN
#define _HPP_BB_MATRIX_BIN
...
namespace bb {
  namespace matrix {
    ...
    __global__
    void bb_bin_histo(int *d_bin_counter, const int rows, const int cols);

    __global__
    void bb_bin_group(int *d_bin_segs_id, int *d_bin_counter, const int rows, const int cols);

    void bb_bin(int *d_bin_segs_id, int *d_bin_counter,
                const int rows, const int cols, int *h_bin_counter) {
      const int num_threads = 256;
      const int num_blocks = ceil((double) rows / (double) num_threads);

      bb_bin_histo << < num_blocks, num_threads >> > (d_bin_counter, rows, cols);
      ...
      bb_bin_group << < num_blocks, num_threads >> > (d_bin_segs_id, d_bin_counter, rows, cols);
